...
export const TeamGridSection = ({ data }: { data?: TeamGridData }) => {
  if (!data) return null;
  ...
  return (
    <section className="relative py-16 lg:py-24 bg-[#fafaf9] overflow-hidden">
      ...
      <div className="w-full max-w-screen-2xl mx-auto px-4 lg:px-8 xl:px-12 relative z-10">
        ...
        {/* Main Content Grid - Full Width for 8 members */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
          ...
          {data.members.map((member) => (
            <Link 
              href={`/team/${member.id}`}
              key={member.id} 
              className="group flex flex-col items-center justify-center text-center p-8 rounded-3xl transition-all duration-300 bg-white border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-2 hover:bg-primary cursor-pointer"
            >
              ...
              <div className="text-sm font-medium relative pb-4 text-[#00695c] transition-colors duration-300 group-hover:text-white/90">
                {member.role}
                
                {/* Thin Line under role */}
                <div className="absolute bottom-1 left-1/2 -translate-x-1/2 w-10 h-0.5 bg-[#00695c] rounded-full transition-opacity duration-300 group-hover:bg-white/50"></div>
              </div>
              
              {member.shortDescription && (
                <p className="text-xs sm:text-sm text-gray-500 transition-colors duration-300 group-hover:text-white/80 mt-1 line-clamp-3">
                  {member.shortDescription}
                </p>
              )}
